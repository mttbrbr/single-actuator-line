...
spacing is uniform at D/53 = 0.1898 m with the current YAML setting. Geometric grading in the outer blocks
stays below approximately 4.2% per cell.

The current production mesh has 30,376,404 cells. Change only
`mesh.cells_per_D` in `config/case.yaml` for a new run; 48 gives 22,525,776
cells and the validator caps the total at 31 million. Every cell must be a
...
(128 by 80 points across the inlet), so one D/16 Mann interval spans about 3.3
D/53 wake-core cells. This adds smaller resolved structures without
